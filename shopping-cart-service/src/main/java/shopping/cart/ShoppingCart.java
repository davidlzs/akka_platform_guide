...
public final class ShoppingCart
  ...
  public static Behavior<Command> create(String cartId, String projectionTag) {
    return Behaviors.setup(
        ctx -> EventSourcedBehavior.start(new ShoppingCart(cartId, projectionTag, ctx), ctx));
  }
  ...
  private ShoppingCart(String cartId, String projectionTag, ActorContext<Command> ctx) {
    super(
  ...
  private ReplyEffect<Event, State> onCheckout(State state, Checkout cmd) {
    if (state.isEmpty()) {
      ctx.getLog().info("Trying to checkout empty shopping cart " + cartId);
      return Effect()
          .reply(cmd.replyTo, StatusReply.error("Cannot checkout an empty shopping cart " + cartId));
    } else {
      return Effect()
  ...
  private CommandHandlerWithReplyBuilderByState<Command, Event, State, State>
    ...
    return newCommandHandlerWithReplyBuilder()
        ...
        .onCommand(
            Checkout.class,
            cmd -> {
              ctx.getLog().info("Checkout on cart entity " + cartId);
              return Effect()
                      .reply(
                              cmd.replyTo,
                              StatusReply.error("Can't checkout already checked out shopping cart " + cartId));
            });
  }
