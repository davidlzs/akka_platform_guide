...
import io.grpc.examples.helloworld.GreeterHandlerFactory;
    ...
    Function<HttpRequest, CompletionStage<HttpResponse>> service =
        ServiceHandler.concatOrNotFound(
            ShoppingCartServiceHandlerFactory.create(grpcService, system),
                GreeterHandlerFactory.create(new GreeterServiceImpl(), system),
            // ServerReflection enabled to support grpcurl without import-path and proto parameters
            ServerReflection.create(
                    Arrays.asList(ShoppingCartService.description, Greeter.description), system));
